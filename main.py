...
class Bot(commands.Bot):
    ...
    async def punish(self, message, login, author, base_seconds, reason):
        # 1) Удаление сообщения через Helix API (уже правильно, но теперь с явным логом ошибки)
        try:
            await message.delete()
        except Exception as e:
            print(f"[!] Не удалось удалить сообщение у {author} в {login}: {e}")
        ...
        if count == 1:
            ...
            # ВАЖНО: раньше здесь отправлялась текстовая команда "/timeout ..." прямо в чат.
            # Twitch перестал поддерживать управление модерацией через такие текстовые
            # IRC-команды — сообщение просто уходило как обычный текст и ничего не делало,
            # без каких-либо ошибок в логах. Теперь тайм-аут выдаётся через официальный
            # Helix API (PartialUser.timeout_user), который реально применяет тайм-аут.
            #
            # Для этого токен бота (TWITCH_OAUTH_TOKEN) должен быть выпущен со scope
            # moderator:manage:banned_users, а для удаления сообщений (message.delete())
            # — со scope moderator:manage:chat_messages. Если тайм-аут не сработает,
            # в логах Render появится конкретная ошибка (Unauthorized/HTTPException),
            # по которой будет понятно, что не так с токеном.
            try:
                ...
                await broadcaster.timeout_user(
                    ...
                    duration=base_seconds * count,
                    reason=reason,
                )
            except Exception as e:
                print(f"[!] Не удалось выдать тайм-аут {author} в {login}: {e}")
