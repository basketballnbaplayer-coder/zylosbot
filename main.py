...
class Bot(commands.Bot):
    ...
    async def roulette(self, ctx):
        login = ctx.channel.name.lower()

        if random.randint(1, 6) != 1:
            await ctx.send(f"@{ctx.author.name}, в этот раз тебе повезло! 🍀")
            return

        if not self.user_id:
            print(f"[!] Рулетка: ID бота ещё не получен, не могу выдать тайм-аут в {login}")
            await ctx.send(f"@{ctx.author.name}, повезло — не смог тебя наказать 😅")
            return
        ...
        try:
            ...
            await ctx.send(f"🔫 @{ctx.author.name} испытал судьбу... и проиграл! Тайм-аут на 1 минуту 😂")
            self.log(login, "roulette", "проигрыш в рулетке — тайм-аут 60 сек", author=ctx.author.name.lower())
        except Exception as e:
            print(f"[!] Рулетка: не удалось выдать тайм-аут {ctx.author.name}: {describe_api_error(e)}")
            await ctx.send(f"@{ctx.author.name}, повезло — не смог тебя наказать 😅")
