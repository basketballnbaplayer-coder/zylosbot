...
ADMIN_PASSWORD_HASH = generate_password_hash(os.getenv("ADMIN_PASSWORD", "asdxze123"))
...
def init_oauth(app):
    ...
    if TWITCH_CLIENT_ID and TWITCH_CLIENT_SECRET:
        oauth.register(
            ...
            client_kwargs={
                "scope": "",
                # Twitch не поддерживает Basic Auth (дефолт authlib) для этого запроса —
                # client_id/secret нужно передавать в теле POST-запроса.
...
def twitch_callback():
    ...
    if "access_token" not in token:
        print(f"[!] Twitch OAuth не вернул access_token. Ответ: {token}")
        flash("Не удалось войти через Twitch (ошибка авторизации). Попробуй ещё раз.")
        return redirect(url_for("auth.login"))
