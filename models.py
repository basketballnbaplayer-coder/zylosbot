...
engine = create_engine(
    ...
    echo=False,
    future=True,
    pool_pre_ping=True,   # проверяет соединение перед использованием — чинит "SSL SYSCALL error: EOF detected"
    pool_recycle=300,     # пересоздаёт соединения старше 5 минут, чтобы Supabase не рвала их первой
)
